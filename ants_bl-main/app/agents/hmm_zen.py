...
from app.lector.lector_hmm import LectorHMM
...
class AgenteHMM(Agente):
    def __init__(self,  web_driver: SeleniumWebDriver, data: DatabaseManager):
        super().__init__( web_driver, data)
        self.instructions = lambda bl: f"%5B%7B%22fill%22%3A%5B%22%23esvcGlobalQuery%22%2C%22{bl.bl_code}%22%5D%7D%2C%7B%22evaluate%22%3A%22esvcGlobalSearch()%22%7D%2C%7B%22wait%22%3A500%7D%2C%7B%22wait_for%22%3A%22%23trackingInfomationDateResultTable%22%7D%2C%7B%22wait%22%3A3000%7D%5D"
        ...
        self.url = "https://www.hmm21.com/e-service/general/trackNTrace/TrackNTrace.do"
    ...
    def guardar_html(self,response,bl):
        ...
        if response.status_code == 200:
            ...
            lector = LectorHMM(html, bl)
            tabla = lector.extraer_tabla_html()

            # Nombre del archivo en S3
            nombre_html = f'html_hmm/{bl.bl_code}_{fecha}.html'
            ...
            # Nombre del archivo en S3
            nombre_error = f'html_hmm/error_{bl.bl_code}_{fecha}_zenrows.json'
